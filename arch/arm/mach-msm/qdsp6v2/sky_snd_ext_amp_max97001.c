...
{
	printk("  [SKY Audio] aud_sub_open");
	return 0;
}

static int aud_sub_release(struct inode *inode, struct file *file)
{
	printk("  [SKY Audio] aud_sub_release");
	return 0;	
}

static long aud_sub_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	printk("  [SKY Audio] aud_sub_ioctl ");//jhpark cmd:%d, arg:%d \n", cmd, arg);
       snd_extamp_api_SetDevice(cmd, arg);
	return 0;
}
...
void snd_extamp_api_Init(void)
{
	...
	if(result)
	{
       	printk(KERN_ERR "  [SKY Audio] misc_register failed.\n");
	}
#endif //jhpark	
