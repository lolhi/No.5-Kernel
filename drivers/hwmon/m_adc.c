...
void msm_adc_wq_work(struct work_struct *work)
{
	...
	case START_OF_CONV:
		...
		//printk("msm_adc_wq_work ::: START_OF_CONV\n");
		#endif
	...
	case END_OF_CONV:
		#ifdef FEATURE_AT1_PMIC_ADC
		//printk("msm_adc_wq_work ::: END_OF_CONV\n");
		#endif
	...
	case END_OF_CALIBRATION:
		#ifdef FEATURE_AT1_PMIC_ADC
		//printk("msm_adc_wq_work ::: END_OF_CALIBRATION\n");
		#endif
		/* for blocking requests, signal complete */
		if (slot->blocking) {
			#ifdef FEATURE_AT1_PMIC_ADC
			//printk("msm_adc_wq_work :::slot->blocking = %d\n",slot->blocking);
			#endif
			complete(&slot->comp);
		} else {
		 ...
		 */
			...
			mutex_unlock(&client->lock);
			#ifdef FEATURE_AT1_PMIC_ADC
			//printk("msm_adc_wq_work :::mutex_unlock\n");
			#endif
			...
			#ifdef FEATURE_AT1_PMIC_ADC
			//printk("msm_adc_wq_work :::slot->compk = %d\n",slot->compk);
			#endif
	...
	case START_OF_CALIBRATION: /* code here to please code reviewers
		...
		#ifdef FEATURE_AT1_PMIC_ADC
		//printk("msm_adc_wq_work ::: START_OF_CALIBRATION\n");					
		#endif
	break;
	}
}
