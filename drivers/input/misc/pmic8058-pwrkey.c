...
static enum hrtimer_restart pmic8058_pwrkey_ignore_timer(struct hrtimer *ignore_timer)
{
        struct pmic8058_pwrkey *pwrkey = container_of(ignore_timer,
                                                struct pmic8058_pwrkey, ignore_timer);

	//printk("Power Key Ignore End %s %d\n",__FUNCTION__,__LINE__);

	enable_irq(pwrkey->key_press_irq);
	enable_irq(pwrkey->key_release_irq);
...
//		printk("Key Release %s %d\n",__FUNCTION__,__LINE__);
	...
	hrtimer_start(&pwrkey->ignore_timer,
			ktime_set(POWER_IGNORE_TIMER / 1000,
			(POWER_IGNORE_TIMER % 1000) * 1000000),
			HRTIMER_MODE_REL);
	disable_irq_nosync(pwrkey->key_press_irq);
	disable_irq_nosync(pwrkey->key_release_irq);
	//printk("Power Key Ignore start %s %d\n",__FUNCTION__,__LINE__);
#endif
	return IRQ_HANDLED;
}
...
// paiksun...
	...
	hrtimer_init(&pwrkey->ignore_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	pwrkey->ignore_timer.function = pmic8058_pwrkey_ignore_timer;
