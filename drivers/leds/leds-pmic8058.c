...
static void pmic8058_led_work(struct work_struct *work)
{
	...
	switch (led->id) {
	case PMIC8058_ID_LED_KB_LIGHT:
#if AT1_BDVER_GE(AT1_TP10) //20110607 bandit set for LED 8mA
		#if 1
		...
		#else
		printk("pmic8058_led_work : %d  led->brightness : %d\n", led->id, led->brightness);
		led_lc_set(led, 5);
		#endif
#else //AT1_BDVER_GE(AT1_TP10)
#if defined(CONFIG_EF33_BOARD) || defined(CONFIG_EF34_BOARD) || defined(CONFIG_EF35_BOARD)  || defined(CONFIG_EF40_BOARD) /*yjw*/
		led_lc_set(led, led->brightness);
#else
		kp_bl_set(led, led->brightness);
#endif
#endif //AT1_BDVER_GE(AT1_TP10)
		break;
	case PMIC8058_ID_LED_0:
	case PMIC8058_ID_LED_1:
	case PMIC8058_ID_LED_2:
