...
{
#if 0 //def FEATURE_SKY_MMC
	if (host->index == 1) {
		if (host->bus_ops != NULL && msm8x60_sdcc_slot_status()) {
			printk("(mmc-debug) micro-sd attached, don't turn off ldo\n");
			return;
		}
		else {
			printk("(mmc-debug) no micro-sd , turn off ldo\n");
		}
	}
#endif
	host->ios.clock = 0;
