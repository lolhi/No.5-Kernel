...
#define MSM_CHARGER_TA_CURRENT_LCDON 700
#endif
#endif
/**
...
void msm_charger_set_current_incall(unsigned int in_call)
{
	...
	if(in_call) {
		chg_current = MSM_CHARGER_TA_CURRENT_INCALL;
		 ...
		 #if 0//def FEATURE_AT1_PMIC_CHARGING
		if(is_lcd_on())
		chg_current = MSM_CHARGER_TA_CURRENT_LCDON;
		else
		#endif
...
static int msm_chg_dequeue_event(struct msm_charger_event **event)
{
	...
	*event = &msm_chg.queue[msm_chg.head];
	...
	msm_chg.queue_count--;
	//qc test
	pr_err("%s dequeue queue_count = %d\n", __func__, msm_chg.queue_count);
	spin_unlock_irqrestore(&msm_chg.queue_lock, flags);
	return 0;
}
			...
			enum msm_hardware_charger_event event)
{
	...
	msm_chg.queue_count++;
	//qc test
	pr_err("%s enqueue tail = %d, queue_count = %d\n", __func__, msm_chg.tail, msm_chg.queue_count);
	spin_unlock_irqrestore(&msm_chg.queue_lock, flags);
	return 0;
}
