...
#if defined(FEATURE_ANDROID_PANTECH_USB_SERIAL_SET_at_8X60)
...
#if defined(FEATURE_FASTER_USB_SERIAL_WRITE) /* jylee 110725 */
static char pantech_serial_number1[MAX_STR_LEN]="AT1"; 
static int serial_written = 0;
#endif
...
static int android_pantech_set_serial_number(void)
{
  ...
  #ifdef FEATURE_NAMING_AT1 /* jylee */
  memcpy(pantech_serial_number, "AT1", strlen("AT1"));
  memcpy(pantech_serial_number + strlen("AT1"), ret.output, *ret.out_len);
  #else
...
{
	#if defined(FEATURE_FASTER_USB_SERIAL_WRITE)
        memcpy(pantech_serial_number1 + strlen("AT1"), buf, 14);
        strings_dev[STRING_SERIAL_IDX].s = pantech_serial_number1;
        serial_written = 1;
	#else
	pantech_serial_number = kmalloc(MAX_STR_LEN, GFP_KERNEL);
	memset(pantech_serial_number, 0x00, MAX_STR_LEN);
...
#if defined(FEATURE_FASTER_USB_SERIAL_WRITE) /*jylee 110725 */
static ssize_t show_sr_done(struct device *dev, struct device_attribute *attr, char *buf)
{
    size_t i;
    i = sprintf(buf, "%d\n", serial_written);
    return i;
...
#if	defined(FEATURE_ANDROID_PANTECH_USB_UEVENT_PATCH)
	int index, found = 0;
#else
	int index;
#endif
