			    ...
			    boolean op_enable)
{
	...
	//down(&msm_fb_onoff_sem);
#endif
	PRINT("[LIVED] msm_fb_blank_sub: blank_mode=%d, mfd->panel_power_on=%d\n",
...
#if AT1_BDVER_GE(AT1_WS20)  //hhs 20110324 5inch
	var->height = 109.2,	/* height of picture in mm */
	var->width = 65.52,	    /* width of picture in mm */
#elif defined(CONFIG_EF33_BOARD) || defined(CONFIG_EF34_BOARD) || defined(CONFIG_EF35_BOARD)  || defined(CONFIG_EF40_BOARD) /*yjw*/
...
#if 0
        // AT1 compile debug
		mutex_lock(&msm_fb_isoctl_gamma);
